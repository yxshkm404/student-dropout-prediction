...
export default function TeacherProfile() {
  ...
  const infoCards = profile ? [
    { icon:"✉️",  label:"Email",   value: profile.email,  color:"#10b981" },
    { icon:"🔖",  label:"Status",  value: profile.status, color: profile.status==="APPROVED"?"#059669":"#d97706" },
    { icon:"📅",  label:"Role",    value: "Teacher",      color:"#0891b2" },
    { icon:"🏫",  label:"System",  value: "EduPredict",   color:"#7c3aed" },
  ] : [];
  ...
  return (
    <div style={{ padding:"36px", minHeight:"100vh", background:"#f1f5f9",
      ...
      <style>{`
        ...
        .edit-btn:hover { transform:translateY(-2px) !important; box-shadow:0 8px 24px rgba(16,185,129,.45) !important; }
      `}</style>
      ...
      <div style={{ flex:1 }}>
        ...
        {loading ? (
          ...
          <>
            {/* Hero card */}
            <div style={{ background:"linear-gradient(135deg,#0a0f2e,#064e3b,#065f46)",
              borderRadius:"20px", padding:"36px", marginBottom:"24px",
              display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"20px",
              boxShadow:"0 8px 32px rgba(16,185,129,.20)",
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"20px" }}>
                ...
                <div>
                  ...
                  <div style={{ marginTop:"10px" }}>
                    <span style={{
                      background: profile.status==="APPROVED" ? "rgba(16,185,129,.30)" : "rgba(245,158,11,.30)",
                      color:      profile.status==="APPROVED" ? "#6ee7b7" : "#fde68a",
                      border:`1px solid ${profile.status==="APPROVED"?"rgba(16,185,129,.5)":"rgba(245,158,11,.5)"}`,
              ...
              <button className="edit-btn" onClick={openEdit}
                style={{ background:"linear-gradient(135deg,#10b981,#059669)", color:"#fff",
                  ...
                  fontSize:"14px", fontWeight:700, cursor:"pointer", fontFamily:"inherit",
                  boxShadow:"0 4px 16px rgba(16,185,129,.35)", transition:"all .25s" }}>
                ✏️ Edit Profile
              </button>
            </div>
            ...
            <div style={{ marginTop:"20px", background:"rgba(16,185,129,.06)",
              border:"1px solid rgba(16,185,129,.20)", borderRadius:"12px",
              padding:"14px 20px", fontSize:"13px", color:"#059669", fontWeight:500,
              animation:"fadeUp .6s ease both .15s", opacity:0, animationFillMode:"forwards" }}>
              🔒 You can update your name, email and password anytime from this page.
            </div>
          </>
        )}
      </div>
